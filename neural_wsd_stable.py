...
if __name__ == "__main__":
    ...
    data, lemma2synsets, lemma2id, synset2id, id2synset = data_ops_final.read_folder_semcor(data, lexicon_mode=lexicon_mode, f_lex=lexicon)
    #random.shuffle(data)
    ...
    def accuracy(logits, labels, lemmas, statistics=False):
    ...
    def new_batch (offset):
        ...
        inputs, seq_lengths, labels, words_to_disambiguate, indices, lemmas, synsets_gold = \
            data_ops_final.format_data(wsd_method, batch, src2id, lemma2synsets, synset2id, seq_width,
    ...
    for step in range(training_iters):
        ...
        if (step % 50 == 0):
            ...
            if step % 10000 == 0:
                accuracy(fetches[3], val_labels, val_lemmas_to_disambiguate, statistics=True)
        else:
        ...
        if (args.save_path != "None" and step % 25000 == 0 and step != 0 or step == 100):
